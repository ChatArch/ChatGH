...
def test_chatgh_run_view_basic(monkeypatch, runner):
    monkeypatch.setattr(
        gh_cli,
        "view_run",
    ...
    result = runner.invoke(cli, ["run", "view", "--run-id", "23494900414"])
...
def test_chatgh_run_logs_basic(monkeypatch, runner):
    monkeypatch.setattr(
        gh_cli,
        "view_job_logs",
    ...
    result = runner.invoke(cli, ["run", "logs", "--job-id", "68373094563", "--tail", "2"])
